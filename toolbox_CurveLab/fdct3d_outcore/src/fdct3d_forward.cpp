...
int fdct3d_forward_angles(double L1, double L2, double L3, int s, int nd, CpxOffTns& O, CpxCrvletOcr& C)
{
  //allocate space
...
int fdct3d_forward_wavelet(double L1, double L2, double L3, int s, CpxOffTns& O, CpxNumTns& W)
{
  //vector<CpxNumTns>& csc = C[s];
  //csc.resize(1);
  ...
  CpxNumTns& T=W;  T.resize(N1,N2,N3);
...
int fdct3d_forward_center(double L1, double L2, double L3, int s, CpxOffTns& O, CpxCrvletOcr& C)
{
  //vector<CpxNumTns>& csc = C[s];
  //csc.resize(1);
...
int fdct3d_forward(int N1, int N2, int N3, int nbscales, int nbdstz_coarse,
					 CpxNumTns& X,				 CpxCrvletOcr& C, CpxNumTns& W)
{
  ...
  //tm1 = time(NULL);  cout<<"fft+shift "<<difftime(tm1,tm0)<<endl;  tm0 = tm1;
  
  vector< vector<double> > fxs, fys, fzs;  vector< vector<int   > > nxs, nys, nzs;
  fdct3d_param(N1,N2,N3, nbscales,nbdstz_coarse, fxs,fys,fzs, nxs,nys,nzs);
  
  int L = nbscales;
  nxs.resize(L-1);  nys.resize(L-1);  nzs.resize(L-1); //remove the last level
  C.setup(nxs, nys, nzs, 10);
  //tm1 = time(NULL);  cout<<"setup "<<difftime(tm1,tm0)<<endl;  tm0 = tm1;
  ...
  }//tm1 = time(NULL);  cout<<"angles "<<difftime(tm1,tm0)<<endl;  tm0 = tm1;
  {
	 int s = L-1;
	 double L1 = 4.0*N1/3.0 / pow2(L-1-s);	 double L2 = 4.0*N2/3.0 / pow2(L-1-s);	 double L3 = 4.0*N3/3.0 / pow2(L-1-s);
	 fdct3d_forward_wavelet(L1,L2,L3,s, O, W);
